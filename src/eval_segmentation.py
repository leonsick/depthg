...
def batched_crf(pool, img_tensor, prob_tensor):
    # Pool can be problematic, so we use a single process
...
def my_app(cfg: DictConfig) -> None:
    ...
    model_name = cfg.model_paths[0].split("/")[-1]

    prefix = "{}/{}".format(model_name, cfg.experiment_name)
    ...
    for model_path in cfg.model_paths:
        wandb_logger = wandb.init(project="stegodepth-eval", name=name, config=config_dict)
        ...
        with Pool(cfg.num_workers + 5) as pool:
            for i, batch in enumerate(tqdm(test_loader)):
                with torch.no_grad():
                    print(f"Iteration {i}") if i < 5 else None
                    img = batch["img"].cuda()
                    label = batch["label"].cuda()
                    print("Data loaded...") if i < 5 else None

                    feats, code1 = par_model(img)
                    feats, code2 = par_model(img.flip(dims=[3]))
                    code = (code1 + code2.flip(dims=[3])) / 2

                    print("Codes Computed...") if i < 5 else None
                    ...
                    print("Running CRF...") if i < 5 else None
                    if cfg.run_crf:
                    ...
                    print("CRF Done...") if i < 5 else None

                    print("Updating Metrics...") if i < 5 else None
